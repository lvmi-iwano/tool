...
static void __init block_to_pages(pmd_t *pmd, unsigned long addr,
										 unsigned long end, unsigned long pfn)
{
	pte_t *old_pte;
	pmd_t new ;
	int i = 0;
	pte_t *pte = tsee_rtks_ro_alloc();
...
#ifdef CONFIG_TRUSTKERNEL_TSEE_RTKS
	if(pmd_block(*pmd) && 0 /* BUG and FIXME: fix it later*/) {
		block_to_pages(pmd, addr, end, pfn);
		return;
	}
#endif
...
#ifdef CONFIG_TRUSTKERNEL_TSEE_RTKS
		pmd = tsee_rtks_ro_alloc();
		if(!pmd)
			pmd = alloc(PTRS_PER_PMD * sizeof(pmd_t));
#else   /* !CONFIG_TRUSTKERNEL_TSEE_RTKS */
		pmd = alloc(PTRS_PER_PMD * sizeof(pmd_t));
#endif
		if (pud_sect(*pud)) {
			/*
...
#ifdef CONFIG_TRUSTKERNEL_TSEE_RTKS
	tsee_rtks_extra_mem = early_alloc(TSEE_RTKS_EXTRA_MEM_SIZE);
	if (!do_memset) {
		memset((void*)TSEE_RTKS_ROBUF_VA, 0, TSEE_RTKS_ROBUF_SIZE);
		do_memset = 1;
	}
#endif
