...
int tsee_main(uint64_t cmd, uint64_t arg0, uint64_t arg1, uint64_t arg2, uint64_t arg3, uint64_t arg4)
{
	printk("tsee(0x%llx)-0x%llu-0x%llx-0x%llx-0x%llx-0x%llx)\n", cmd, arg0, arg1, arg2, arg3, arg4);
	switch(cmd) {
		case TSEE_RTKS_INIT:
			printk("tsee rtks init done\n");
			memcpy(&tsee_init, (uint64_t*)arg0, sizeof(tsee_rtks_init_t));
			break;
		case TSEE_RTKS_DEF_INIT:
			printk("tsee rtks definit done\n");
			break;

		//  Page table management

		case TSEE_RTKS_PGD_SET:
			//TODO: check both pointer
			printk("tsee rtks pgd set\n");
			*(uint64_t*)arg0 = arg1;
			dsb(ishst);
			isb();
			break;
		case TSEE_RTKS_PMD_SET:
			//TODO: check both pointer
			printk("tsee rtks pmd set\n");
			*(uint64_t*)arg0 = arg1;
			dsb(ishst);
			isb();
			break;
		case TSEE_RTKS_PTE_SET:
			//TODO: check both pointers
			printk("tsee rtks pte set\n");
			*(uint64_t*)arg0 = arg1;
			dsb(ishst);
			isb();
			break;
		case TSEE_RTKS_PGD_FREE:
			printk("tsee rtks pgd free\n");
			break;
		case TSEE_RTKS_PGD_NEW:
			printk("tsee rtks pgd new\n");
			break;
		default:
			printk("TSEE Error: unknown cmd for tsee\n");
			break;
	}
	return 0;
}
...
int tsee_ns_main(uint32_t cmd, uint32_t arg0, uint32_t arg1, uint32_t arg2, uint32_t arg3, uint32_t arg4)
{
	...
	switch(cmd) {
		case TSEE_RTKS_MEM_PROTECT_NS:
			...
			if(((arg0 >= tsee_init.kmem_start + tsee_init.kmem_end) || (arg0 + arg1 <= tsee_init.kmem_start)) &&
			   ((arg0 >= tsee_init.kmem_start2 + tsee_init.kmem_end2) || (arg0 + arg1 <= tsee_init.kmem_start2)) &&
			   ((arg0 >= tsee_init.reemem_shr_start + tsee_init.reemem_shr_end) || (arg0 + arg1 <= tsee_init.reemem_shr_start)) ) {
...
int tsee_main(uint32_t cmd, uint32_t arg0, uint32_t arg1, uint32_t arg2, uint32_t arg3, uint32_t arg4)
{
	...
	switch(cmd) {
		...
		case TSEE_RTKS_LOCK_DOWN:
			...
			sm_lock_down();
			break;
		case TSEE_RTKS_VBAR_SET:
			DMSG("tsee rtks set vbar:0x%x\n", arg0);
