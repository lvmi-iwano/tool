...
#include "gf3208-common.h"
#include "gf3208-milanf.h"
#include "gf3208-regs.h"
...
{   
   //   {  "POWER",  KEY_POWER,  	KEY_POWER/*26*/},
      {  "HOME" ,   KEY_HOME,   	KEY_F11/*3*/},
      {  "MENU" ,   KEY_MENU,   	KEY_MENU/*82*/ },
      {  "BACK" ,   KEY_BACK,   	KEY_BACK/*/4*/ },
      //{  "UP"   ,      KEY_UP,     	KEY_UP/*19*/ },
    //  {  "DOWN" ,  KEY_DOWN,   	KEY_DOWN/*20*/ },
     // {  "LEFT" ,     KEY_LEFT,   	KEY_LEFT/*59*/ },
    //  {  "RIGHT",   KEY_RIGHT,  	KEY_RIGHT/*60*/ },
      //{  "FORCE",  KEY_F9,     		KEY_F9/*139*/},  //modify
   //   {  "CLICK",  KEY_F19,    		KEY_F19/*140*/},
    //  {  "CAM",      KEY_F11,		68 },	
};
...
*********************************************************/
#define 	GF_PWN_PIN         (GPIO99 | 0x80000000) //GPIO_FIGERPRINT_RST
...
//	unsigned char config_type = 0;
	...
	case GF_IOC_CONFIG:
        ...
        if (cfg_len > 0) {
            p_cfg = (struct gf_configs*)kzalloc(cfg_len, GFP_KERNEL);
            if(p_mode_cfg == NULL) {
...
{
	gf_dev_t *gf_dev = &gf;
	//printk("gf3208 fingerprint_eint_interrupt_handler 6.17\n");
#if GF_FASYNC
	if(gf_dev->async) {
...
static int fingerprint_dts_probe(struct platform_device *dev)
{
	...
	gf_debug(DEFAULT_DEBUG, "gf3208...%s %d\n",__func__,__LINE__);
...
static int  gf_probe(struct spi_device *spi)
{
	...
	FUNC_ENTRY();
	gf_debug(DEFAULT_DEBUG, "driver version is %s.\n",GF_DRIVER_VERSION);
	...
	//while(1){
		for(tt=0;tt<3;tt++) {
			...
			gf_debug(DEFAULT_DEBUG, "%s chip id is chip_id_2:0x%04x,chip_id_1:0x%04x\n",__func__,chip_id_2,chip_id_1);
...
//	node = of_find_matching_node(node, fingerprint_of_match);
	...
	if (node) {
		...
		gf_debug(DEFAULT_DEBUG, "gx368_gf3208 fingerprint_irq request_irq ret=%d\n   ",ret);
		...
		//printk("gx368/gf3208 fingerprint_irq ret2=%d\n   ",ret);		
	}
...
static int gf_suspend_test(struct device *dev)
{
   ...
   gf_debug(DEFAULT_DEBUG, "gf3208: %s\n", __func__);
    return 0;
}
...
static struct spi_board_info spi_board_devs[] __initdata = {
	[0] = {
		...
		.bus_num = 0,
		.chip_select=FINGERPRINT_GF3208_GOODIX_CS,
		.mode = SPI_MODE_0,
