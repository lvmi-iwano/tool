...
//ether add sheep begin 20160526
#include <linux/notifier.h>
#include <linux/fb.h>
//ether add sheep end 20160526
...
static long gfx1xm_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case GFX1XM_IOC_SLEEP:
		...
		mutex_unlock(&gfx1xm_dev->fb_lock);
	    break;
		////ether add sheep add sleep begin 20160526
	case GF_IOC_PM_FBCABCK:
		printk("go to ioctl fb_notify-----tsy");
		__put_user(gfx1xm_dev->fb_black, (u8 __user *) arg);
		break;
		////ether add sheep add sleep end 20160526
	default:
...
//ether add sleep begin 20160526
...
#if GFX1XM_FASYNC
                        if (gfx1xm_dev->async) {
                            kill_fasync(&gfx1xm_dev->async, SIGIO,
                                POLL_IN);
							pr_info("%s kill sigio power down\n", __func__);
                        }
#endif
...
static int  gfx1xm_probe(struct spi_device *spi)
{
	...
	#if 0
	spi->dev.of_node=of_find_compatible_node(NULL, NULL, "mediatek,mt6755-finger");
...
static struct spi_board_info spi_board_devs[] __initdata = {
	[0] = {
		...
		.bus_num = 0,
		.chip_select=FINGERPRINT_GFX1XM_GOODIX_CS,
		.mode = SPI_MODE_0,
