...
static int32_t o2_backoff_twostep_support = 0;
#endif
	...
	if ( vbus_volt > (ADJI_JUDG_VBUSVOLT_LOWTH + 100) ) {
		...
		switch(adji_ret) {
			case ADJI_RET_NOISE_OR_APOWERLOW:
				...
				if(BW_ABS(vbus_volt,tmp) < 100 && BW_ABS(cc_val,prevcc) < 200) {
...
#endif
{
	...
	//check VDC_PR
	bluewhale_read_byte(charger, REG_VBUS_STATUS, &data_status);                 
...
static int ir_compensation(struct bluewhale_charger_info *charger, int cv_val)
{
	...
	ir = adji_get_bat_curr() * charger->pdata->inpedance /1000;
	if (cv_val)
	...
	printk(KERN_WARNING"calculate IR volt  is %d\n", charge_volt);
...
//	if (is_ta_connect == KAL_TRUE)
	if (mtk_pep_get_is_connect() == true)
