...
static u32 charging_set_cv_voltage(void *data)
{
	...
	battery_log(BAT_LOG_FULL,"[charging_set_cv_voltage] set CV cv_value: %dmV\n", cv_value);
	if (cv_value < 4375) cv_value = 4375;
...
static u32 charging_set_current(void *data)
{
...
#if defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT)	
	//if(current_value > 3000 && is_ta_connect == KAL_TRUE) //3A
	if(current_value > 3000 && mtk_pep_get_is_connect() == true) //3A
#else
	if(current_value > 3000) //3A
#endif
	{
#if 0	
	sub_current_value = current_value / 2;
		...
		current_value = current_value / 200 + 200;
#else
		//only for test
...
static u32 charging_set_input_current(void *data)
{
	...
	unsigned int sub_current_value = 0;
#if defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT)	
	if(current_value>=2000 && mtk_pep_get_is_connect() == true) //2A
#else
	if (0)
#endif
	{
		sub_current_value = current_value/2; //1200  改9V2A
...
static u32 charging_set_ta_current_pattern(void *data)
{
	...
	battery_log(BAT_LOG_CRTI, "charging_set_ta_current_pattern: 2016/08/15\n");
...
#if defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT)	
	if (mtk_pep_get_is_connect() == false && KAL_TRUE == enable)
	{
		oz115_sub_stop_charge_extern(0); //set SUS to disable charger
		return 1;
	}
